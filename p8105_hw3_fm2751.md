...
accel = 
  ...
  pivot_longer(
    ...
    names_to = "activity_time_in_mins",
    values_to = "activity_counts") %>% 
  mutate(
    activity_time_in_mins = as.numeric(activity_time_in_mins),
    week = as.factor(week),
...
variables, they are: week, day_id, day, activity_time_in_mins,
activity_counts, weekday_vs_weekend. It’s worth pointing out that
activity time is recorded in minutes, and we added a new variable
`weekday_vs_weekend` to the original dataset to indicate whether the day
is a weekday or a weekend, which can be very helpful in the following
analyses.
